...
/// When writing a match expression against `TransformationProgressUpdateStatus`, it is important to
...
/// }
/// ```
/// The above code demonstrates that when `transformationprogressupdatestatus` represents
/// `NewFeature`, the execution path will lead to the second last match arm,
/// even though the enum does not contain a variant `TransformationProgressUpdateStatus::NewFeature`
/// in the current version of SDK. The reason is that the variable `other`,
/// created by the `@` operator, is bound to
/// `TransformationProgressUpdateStatus::Unknown(UnknownVariantValue("NewFeature".to_owned()))`
/// and calling `as_str` on it yields `"NewFeature"`.
/// This match expression is forward-compatible when executed with a newer
/// version of SDK where the variant `TransformationProgressUpdateStatus::NewFeature` is defined.
/// Specifically, when `transformationprogressupdatestatus` represents `NewFeature`,
/// the execution path will hit the second last match arm as before by virtue of
/// calling `as_str` on `TransformationProgressUpdateStatus::NewFeature` also yielding
/// `"NewFeature"`.
///
...
impl ::std::str::FromStr for TransformationProgressUpdateStatus {
    type Err = ::std::convert::Infallible;

    fn from_str(s: &str) -> ::std::result::Result<Self, <Self as ::std::str::FromStr>::Err> {
        ::std::result::Result::Ok(TransformationProgressUpdateStatus::from(s))
    }
}
impl TransformationProgressUpdateStatus {
