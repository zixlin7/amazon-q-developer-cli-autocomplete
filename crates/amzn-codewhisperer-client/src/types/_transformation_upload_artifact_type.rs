...
/// When writing a match expression against `TransformationUploadArtifactType`, it is important to
...
/// }
/// ```
/// The above code demonstrates that when `transformationuploadartifacttype` represents
/// `NewFeature`, the execution path will lead to the second last match arm,
/// even though the enum does not contain a variant `TransformationUploadArtifactType::NewFeature`
/// in the current version of SDK. The reason is that the variable `other`,
/// created by the `@` operator, is bound to
/// `TransformationUploadArtifactType::Unknown(UnknownVariantValue("NewFeature".to_owned()))`
/// and calling `as_str` on it yields `"NewFeature"`.
/// This match expression is forward-compatible when executed with a newer
/// version of SDK where the variant `TransformationUploadArtifactType::NewFeature` is defined.
/// Specifically, when `transformationuploadartifacttype` represents `NewFeature`,
/// the execution path will hit the second last match arm as before by virtue of
/// calling `as_str` on `TransformationUploadArtifactType::NewFeature` also yielding `"NewFeature"`.
///
...
impl ::std::str::FromStr for TransformationUploadArtifactType {
    type Err = ::std::convert::Infallible;

    fn from_str(s: &str) -> ::std::result::Result<Self, <Self as ::std::str::FromStr>::Err> {
        ::std::result::Result::Ok(TransformationUploadArtifactType::from(s))
    }
}
impl TransformationUploadArtifactType {
