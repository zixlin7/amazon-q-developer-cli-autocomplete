    ...
    > {
        ...
        let mut request_builder = {
            ...
            builder = _header_serialization_settings.set_default_header(
                ...
                "AWSCodeWhispererService.TagResource",
            );
            builder
        };
...
impl TagResourceError {
    ...
    /// Creates the `TagResourceError::Unhandled` variant from an
    ...
    /// Returns `true` if the error kind is `TagResourceError::ThrottlingError`.
    pub fn is_throttling_error(&self) -> bool {
        matches!(self, Self::ThrottlingError(_))
    }

    /// Returns `true` if the error kind is `TagResourceError::ValidationError`.
    ...
    /// Returns `true` if the error kind is `TagResourceError::AccessDeniedError`.
