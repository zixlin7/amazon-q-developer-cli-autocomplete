    ...
    > {
        ...
        let mut request_builder = {
            ...
            builder = _header_serialization_settings.set_default_header(
                ...
                "AWSCodeWhispererService.UpdateProfile",
            );
            builder
        };
        let body = ::aws_smithy_types::body::SdkBody::from(
            crate::protocol_serde::shape_update_profile::ser_update_profile_input(&input)?,
        );
...
impl UpdateProfileError {
    ...
    /// Creates the `UpdateProfileError::Unhandled` variant from an
    ...
    /// Returns `true` if the error kind is `UpdateProfileError::ValidationError`.
    pub fn is_validation_error(&self) -> bool {
        matches!(self, Self::ValidationError(_))
    }

    /// Returns `true` if the error kind is `UpdateProfileError::AccessDeniedError`.
    pub fn is_access_denied_error(&self) -> bool {
        matches!(self, Self::AccessDeniedError(_))
    }

    /// Returns `true` if the error kind is `UpdateProfileError::InternalServerError`.
    pub fn is_internal_server_error(&self) -> bool {
        matches!(self, Self::InternalServerError(_))
    }

    /// Returns `true` if the error kind is `UpdateProfileError::ThrottlingError`.
    pub fn is_throttling_error(&self) -> bool {
        matches!(self, Self::ThrottlingError(_))
    }

    /// Returns `true` if the error kind is `UpdateProfileError::ConflictError`.
    pub fn is_conflict_error(&self) -> bool {
        matches!(self, Self::ConflictError(_))
    }

    /// Returns `true` if the error kind is `UpdateProfileError::ResourceNotFoundError`.
