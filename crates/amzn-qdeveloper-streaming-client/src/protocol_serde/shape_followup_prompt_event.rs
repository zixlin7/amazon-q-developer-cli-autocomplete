...
pub(crate) fn de_followup_prompt_event_payload(
    input: &[u8],
) -> ::std::result::Result<crate::types::FollowupPromptEvent, ::aws_smithy_json::deserialize::error::DeserializeError> {
    let mut tokens_owned =
        ::aws_smithy_json::deserialize::json_token_iter(crate::protocol_serde::or_empty_doc(input)).peekable();
    let tokens = &mut tokens_owned;
    let result =
        crate::protocol_serde::shape_followup_prompt_event::de_followup_prompt_event(tokens)?.ok_or_else(|| {
...
pub(crate) fn de_followup_prompt_event<'a, I>(
    ...
    Option<crate::types::FollowupPromptEvent>,
                ...
                match tokens.next().transpose()? {
                    ...
                    Some(::aws_smithy_json::deserialize::Token::ObjectKey { key, .. }) => {
                        match key.to_unescaped()?.as_ref() {
                            "followupPrompt" => {
                                builder = builder.set_followup_prompt(
                                    crate::protocol_serde::shape_followup_prompt::de_followup_prompt(tokens)?,
                                );
                            },
