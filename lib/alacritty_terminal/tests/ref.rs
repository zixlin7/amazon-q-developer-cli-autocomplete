...
fn ref_test(dir: &Path) {
    ...
    let grid: Grid<Cell> = serde_json::from_str(&serialized_grid).unwrap();
    let ref_config: RefConfig = serde_json::from_str(&serialized_cfg).unwrap();

    let mut terminal = Term::new_test(size, Mock, ref_config.history_size as usize);
