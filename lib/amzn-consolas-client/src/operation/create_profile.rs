    ...
    > {
        ...
        let mut request_builder = {
            ...
            builder = _header_serialization_settings.set_default_header(
                ...
                "AWSCodeWhispererService.CreateProfile",
            );
            builder
        };
        let body = ::aws_smithy_types::body::SdkBody::from(
            crate::protocol_serde::shape_create_profile::ser_create_profile_input(&input)?,
        );
...
impl CreateProfileError {
    ...
    /// Creates the `CreateProfileError::Unhandled` variant from an
    ...
    /// Returns `true` if the error kind is `CreateProfileError::InternalServerError`.
    pub fn is_internal_server_error(&self) -> bool {
        matches!(self, Self::InternalServerError(_))
    }

    /// Returns `true` if the error kind is `CreateProfileError::ThrottlingError`.
    pub fn is_throttling_error(&self) -> bool {
        matches!(self, Self::ThrottlingError(_))
    }

    /// Returns `true` if the error kind is `CreateProfileError::ValidationError`.
    pub fn is_validation_error(&self) -> bool {
        matches!(self, Self::ValidationError(_))
    }

    /// Returns `true` if the error kind is `CreateProfileError::ConflictError`.
    pub fn is_conflict_error(&self) -> bool {
        matches!(self, Self::ConflictError(_))
    }

    /// Returns `true` if the error kind is `CreateProfileError::ResourceNotFoundError`.
    pub fn is_resource_not_found_error(&self) -> bool {
        matches!(self, Self::ResourceNotFoundError(_))
    }

    /// Returns `true` if the error kind is `CreateProfileError::AccessDeniedError`.
