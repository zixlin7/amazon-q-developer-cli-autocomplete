    ...
    > {
        ...
        let mut request_builder = {
            ...
            builder = _header_serialization_settings.set_default_header(
                ...
                "AWSCodeWhispererService.UntagResource",
            );
            builder
        };
...
impl UntagResourceError {
    ...
    /// Creates the `UntagResourceError::Unhandled` variant from an
    ...
    /// Returns `true` if the error kind is `UntagResourceError::ThrottlingError`.
    pub fn is_throttling_error(&self) -> bool {
        matches!(self, Self::ThrottlingError(_))
    }

    /// Returns `true` if the error kind is `UntagResourceError::ValidationError`.
    ...
    /// Returns `true` if the error kind is `UntagResourceError::AccessDeniedError`.
