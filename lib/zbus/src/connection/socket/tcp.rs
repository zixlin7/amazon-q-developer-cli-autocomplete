...
impl WriteHalf for Arc<Async<TcpStream>> {
    async fn sendmsg(&mut self, buf: &[u8], #[cfg(unix)] fds: &[BorrowedFd<'_>]) -> io::Result<usize> {
        #[cfg(unix)]
        if !fds.is_empty() {
...
impl ReadHalf for tokio::net::tcp::OwnedReadHalf {
    ...
    async fn peer_credentials(&mut self) -> io::Result<crate::fdo::ConnectionCredentials> {
        ...
        crate::Task::spawn_blocking(move || win32_credentials_from_addr(&peer_addr), "peer credentials").await
    }
}
